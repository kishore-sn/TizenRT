...
void smp_init(void)
{
	...
	/* We should check whether it is a warm boot here
	   If yes, we should send SGI1
	 */
#ifdef CONFIG_CPU_HOTPLUG
	/* In our case, CONFIG_SMP_NCPUS is always 2, thus
	xCoreID will be identified as the last core we have
	from above loop, so we will use xCoreID here
	*/
	xCoreID = 1;
	lldbg("up_get_secondary_cpu_state[%d] = %d CPU_WAKE_FROM_SLEEP = %d\n", xCoreID, up_get_secondary_cpu_state(xCoreID), CPU_WAKE_FROM_SLEEP);
	if (up_get_secondary_cpu_state(xCoreID) == CPU_WAKE_FROM_SLEEP) {
		os_smp_start();
	}
#endif
}
